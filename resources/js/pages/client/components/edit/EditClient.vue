...
export default {
  ...
  setup() {
    ...
    const form = ref({
      ...
      specialties: client.value.specialties,
      description: client.value.description,
      country_id: (client.value.country?.id) || client.value.country_id,
      city_id: (client.value.city?.id) || client.value.city_id,
      state_id: (client.value.state?.id) || client.value.state_id,
      source: client.value.source,
    ...
    const submit = () => {
      if (validate(form)) {
        store.dispatch('Client/updateClient', form.value).then(() => {
    ...
    const close = () => {
      store.commit('Client/setIsUpdating', false)
    }
