<template>
  ...
  >
    <VCard class="h-100 d-flex flex-column justify-space-between">
      <div>
        <VCardTitle class="headline d-flex align-center justify-space-between"> Filters </VCardTitle>
        <VDivider />
      ...
      <div>
        ...
        <VCardActions>
          ...
          <VBtn @click="handleFilters"> Done </VBtn>
        </VCardActions>
      </div>
    </VCard>
  </VNavigationDrawer>
...
export default {
  ...
  computed: { ...mapGetters('TaskType', ['isLoading', 'filterItems']) },

  methods: {
    ...mapActions('TaskType', ['fetchAllTaskTypes']),
    ...mapMutations('TaskType', ['setIsLoading', 'setHasFilter', 'setFilterItems']),

    getResults() {
      this.query.page = 1
      this.fetchAllTaskTypes(this.query)
    ...
    async clearFilter() {
      try {
        ...
        await this.fetchAllTaskTypes(this.query)
