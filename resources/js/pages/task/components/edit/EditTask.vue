...
export default {
  ...
  setup() {
    ...
    const task = computed(() => store.getters['Task/task'])
    ...
    const form = ref({
      ...
      language_id: task.value.language_id,
      client_id: task.value.client_id,
      country_id: task.value.country_id,
      city_id: task.value.city_id,
      state_id: task.value.state_id,
    })

    description.editor = task.value.description || ''
    ...
    const submit = () => {
      if (validate(form)) {
        ...
        store.dispatch('Task/updateTask', formData).then(() => {
    ...
    const close = () => {
      store.commit('Task/setIsUpdating', false)
    }
