...
export default {
  ...
  setup() {
    ...
    const form = ref({
      ...
      parent_id: category.value.category?.id || category.value.parent_id,
      language_id: category.value.language?.id || category.value.language_id,
      task_type_id: category.value.task_type?.id || category.value.task_type_id,
    ...
    const submit = () => {
      if (validate(form)) {
        store.dispatch('Category/updateCategory', form.value).then(() => {
    ...
    const close = () => {
      store.commit('Category/setIsUpdating', false)
    }
